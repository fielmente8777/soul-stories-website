  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center">
      <div className="relative p-4 w-full max-w-7xl">
        ...
        <button
          onClick={closePopup}
          className="absolute z-40 right-4 -top-4 bg-red-500 text-white w-8 h-8 rounded-full"
        >
          ✕
        </button>
        ...
        {images.length > 1 && (
          <div className="flex lg:justify-between gap-4 absolute lg:left-1/2 left-4 lg:-translate-x-1/2 lg:top-1/2 top-0 -translate-y-1/2 w-full">
            <button onClick={prevImage} className="rounded-full bg-white text-primary aspect-square w-10 text-4xl flex items-center justify-center">
              <MdArrowLeft />
            </button>
            <button onClick={nextImage} className="rounded-full bg-white text-primary aspect-square w-10 text-4xl flex items-center justify-center">
              <MdArrowRight />
            </button>
          </div>
        )}
