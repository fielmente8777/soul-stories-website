...
const LandingNavBar = () => {
  ...
  // Weather state 30.438097377179943, 78.19056825767129
  const { temp } = useWeather({
    longitudeAndLatitude: "30.438097377179943, 78.19056825767129",
  });
    ...
    >
      <nav
        className={` hidden md:block py-4 ${
          pathName === "/hotel/" ? "bg-primary" : "bg-[#F9FFEB] text-primary!"
        }`}
      >
        <Container>
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-2">
                ...
                <Link href={navUpper[2].href} className="flex flex-col font-semibold">
              ...
              <div
                className={`w-[1px] h-10 ${
                  pathName === "/hotel/" ? "bg-white" : "bg-primary"
                }`}
              />
              <ul className="flex items-center gap-4">
                {footerData.lists[1].links.map((link, index) => (
                  <li key={index}>
